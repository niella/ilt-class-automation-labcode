# Disable monitoring for a host in Zabbix
...
if monitoring == "on":
    ...
    if not response.ok:
        print("Error disabling host: Response code %s: %s" % (response.status_code, response.text))
        raise Exception("Request error occured")

    print("Host disabled: Response code %s: %s" % (response.status_code, response.text))